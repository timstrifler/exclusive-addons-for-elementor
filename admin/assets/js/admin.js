jQuery(document).ready(function($) {
	...
	$('.exad-checkbox input[type="checkbox"]').on( 'click', function() {
		headerSaveBtn.addClass( 'exad-save-now' );
		footerSaveBtn.addClass( 'exad-save-now' );
		...
		if( $(this).hasClass('exad-save-now') ) {
			$.ajax( {
				...
				success: function( response ) {
					...
					$('.exad-header-right').prepend('<span class="exad-settings-saved">Settings Saved</span>').fadeIn('slow');
					
					headerSaveBtn.removeClass( 'exad-save-now' );
					footerSaveBtn.removeClass( 'exad-save-now' );
