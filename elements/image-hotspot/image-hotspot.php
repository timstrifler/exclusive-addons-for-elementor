...
class Exad_Image_Hotspot extends Widget_Base {
    ...
    protected function _register_controls() {
        ...
        $repeater->start_controls_tab('tab_tooltip', ['label' => __('Content', 'exclusive-addons-elementor')]);
        ...
        $repeater->start_controls_tab( 'tab_content', [ 'label' => __( 'Type', 'exclusive-addons-elementor' ) ] );
        ...
        $repeater->add_control(
            ...
            [
                ...
                'label_block' => true,
                'default'     => __('Hi', 'exclusive-addons-elementor'),
                'condition'   => [
        ...
        $this->add_control(
            ...
            [
                ...
                'title_field' => '{{{ exad_tooltip_content }}}'
            ]
        );
    ...
    protected function render() {
        ...
        $infobox_image     = $this->get_settings_for_display( 'exad_hotspot_image' );
        $infobox_image_url = Group_Control_Image_Size::get_attachment_image_src( $infobox_image['id'], 'exad_hotspot_image_size', $settings );
        if ( empty( $infobox_image_url ) ) {
            $infobox_image_url = $infobox_image['url'];
        }  else {
            $infobox_image_url = $infobox_image_url;
        } 
        echo '<div class="exad-hotspot '.esc_attr( $settings['exad_hotspot_animation_type'] ).'">';
            echo '<img src="'.esc_url( $infobox_image_url ).'" alt="'.Control_Media::get_image_alt( $settings['exad_hotspot_image'] ).'">';
            if( is_array( $settings['exad_hotspots'] ) ):
                foreach( $settings['exad_hotspots'] as $item ) :
                    $this->add_render_attribute( 'exad_hotspot', 'class', 'exad-hotspot-dot elementor-repeater-item-' . esc_attr( $item['_id'] ) );    
                    echo '<div class="exad-hotspot-item">';
                        echo '<div '.$this->get_render_attribute_string( 'exad_hotspot' ).'>';
                            if ( 'icon' == $item['exad_hotspot_type'] && !empty( $item['exad_hotspot_icon']['value'] ) ) {
                                echo '<div class="exad-hotspot-dot-icon">';
                                    Icons_Manager::render_icon( $item['exad_hotspot_icon'], [ 'aria-hidden' => 'true' ] );
                                echo '</div>';
                            }
                            if ( $item['exad_hotspot_type'] == 'text' ) {
                                printf( '<div class="exad-hotspot-dot-icon"><span>%1$s</span></div>', esc_html( $item['exad_hotspot_text'] ) );
                            }
                            if ( $item['exad_tooltip'] == 'yes' ) {
                                echo '<div class="exad-hotspot-tooltip">';
                                    echo '<h6>'.wp_kses_post( $item['exad_tooltip_content'] ).'</h6>';
                                echo '</div>';
                            }
                        echo '</div>';
                    echo '</div>';
